var count = 0, Controller = "FreightDespatch";
...
function Loaddatatable(type) {
    var urlpath = base + Controller + "/GetFreightDespatchData";
...
function TableRow(result) {
    ...
    if (result.length > 0) {
        ...
        $(olist).each(function (index, element) {
            let color = '', btn = '', date = '', podlink='', trcolor='';
            ...
            content += '<tr '+ trcolor +'>' +
                ...
                '<td>' + element.Box + '<input type="hidden" value="' + element.Total + '" /></td>' +
                '<td>' + element.DeliveryMethod + '<input type="hidden" value="' + element.SaleType + '" /></td>' +
...
$(document).off('click', '.checkList').on('click', '.checkList', function () {
   // let Id = $(this)[0].id;
    ...
    let stock = tblRow[0].cells[9].childNodes[1].value;  //tblRow[0].cells[1].childNodes[1].children[0].value;
    let type = tblRow[0].cells[10].childNodes[1].value;
...
$331('#btnPopSave').on('click', function () {
    ...
    $("#PODModal").css("display", "none");
       data.append('dataList', JSON.stringify(obj));
        ...
        var urlpath = base + Controller+ "/SaveFreightDespatch";
        $331.ajax({
            beforeSend: function () { $.blockUI(); },
            complete: function () { $.unblockUI(); },
            //type: "POST",
            ...
            //contentType: "application/json;charset=utf-8",
            //dataType: "JSON",
            //data: JSON.stringify({ "objList": obj, "files": files }),
            type: "POST",
            url: urlpath,
...
$('#btnRefresh').on('click', function () {
    if(SaveValidation() == true)
    {
        ...
        $.msgbox("<span style='font-weight:bold; color:#FF0000;'>WARNING!!!</span> Are you sure that you want to Booked Cancel.!", {
            ...
            if (result == "Yes") {
                var obj = GetSaveObject();
                var urlpath = base + Controller + "/AwatingBookedCancel";
                $.ajax({
                    beforeSend: function () { $.blockUI(); },
                    complete: function () { $.unblockUI(); },
                    type: "POST",
