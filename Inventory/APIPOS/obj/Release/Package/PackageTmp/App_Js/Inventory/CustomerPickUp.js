var count = 0, Controller = "CustomerPickUp";
...
function Loaddatatable(type) {
    var urlpath = base + Controller + "/GetCustomerPickUpData";
...
function TableRow(result) {
    ...
    if (result.length > 0) {
        ...
        $(olist).each(function (index, element) {
            ...
            {
                element.Comment = "Awaiting Despatch";
            ...
            content += '<tr ' + trcolor + '>' +
                //'<td >' + (index + 1) + '<input type="hidden" value="' + element.IHeadId + '" /></td>' +  element.PostalCode
...
$331('#btnPopSave').on('click', function () {
    ...
    var urlpath = base + Controller + "/SaveCustomerPickup";
    $331.ajax({
        beforeSend: function () { $.blockUI(); },
        complete: function () { $.unblockUI(); },
        type: "POST",
...
//$331('#btnRefresh').on('click', function () {
