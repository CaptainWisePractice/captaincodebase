var count = 0, Controller = "LocalDelivery", FreightController = "FreightDespatch";
...
function Loaddatatable(type) {
    var urlpath = base + Controller + "/GetLocalDeliveryData";
    ...
    if (result.length > 0) {
        ...
        $(olist).each(function (index, element) {
            ...
            content += '<tr ' + trcolor + '>' +
                ...
                '<td>' + element.DayName + '<input type="hidden" value="' + element.SaleType + '" /></td>' +
...
$(document).off('click', '.checkList').on('click', '.checkList', function () {
    ...
    let qty = tblRow[0].cells[7].textContent;
    let stock = tblRow[0].cells[8].childNodes[1].value;  //tblRow[0].cells[1].childNodes[1].children[0].value;
...
$331('#btnPopSave').on('click', function () {
    ...
    var urlpath = base + Controller + "/SaveLocalDeliveryDespatch";
    $331.ajax({
        beforeSend: function () { $.blockUI(); },
        complete: function () { $.unblockUI(); },
        type: "POST",
