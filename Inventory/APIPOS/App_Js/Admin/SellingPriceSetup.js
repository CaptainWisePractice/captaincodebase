var Controller = "SellingPriceSetup", ItemController = "ItemEntry", arryHeadLst = [];
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Error != null) {
                ...
                debugger;
                arryHeadLst = [];
                arryHeadLst = result.listComboData;
    ...
    if (result != undefined) {
            ...
            $(result).each(function (index, element) {
                content += '<tr>' +
                    ...
                    '<td class="price2"><input type="hidden" class="Itemtype" value="' + element.Itemtype + '"/>' + element.Offer2 + '</td>' +
...
function ClearData() {
    ...
    $('#hfId').val('0');
    $('#btnSave').val('Save');
    LoadDropdown(arryHeadLst, $('#ddlHead'));
}

$(document).off('click', '.edit').on('click', '.edit', function () {
    let tblRow = $(this).closest('tr');
    
    let Itemtype = $(tblRow).find('.Itemtype').val();
    var serLst = $.grep(arryHeadLst,
        function (x) {
            if (x.label == Itemtype) {
                return x;
            }
        });
    LoadDropdown(serLst, $('#ddlHead'));
