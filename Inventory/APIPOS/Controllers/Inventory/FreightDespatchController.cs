...
{
    [NoCache]
    public class FreightDespatchController : Controller
    {
        ...
        public ActionResult FreightDespatch()
        {
            return View();
        }
        public JsonResult GetFreightDespatchData(string FromDate, string Todate,string type)
        {
            _objWrapper = new WrapperDispatch();
            _objWrapper = _objDal.GetFreightDespatchData(FromDate, Todate, type);
            JsonResult json = Json(_objWrapper, JsonRequestBehavior.AllowGet);
            json.MaxJsonLength = 2147483644;
            return json;
        }
        [HttpPost]
        public JsonResult SaveFreightDespatch(string dataList)
        {
            ...
            if (Request.Files.Count > 0)
            {
                try
                {
                    ...
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        string subPath = "/Uploads/FreightDespatch/";
